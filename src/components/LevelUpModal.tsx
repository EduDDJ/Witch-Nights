...
export const LevelUpModal: React.FC<LevelUpModalProps> = ({
  ...
  const getShootingTypeLabel = (type: string) => {
    ...
    switch (type) {
      case 'NEAREST_ENEMY':
        return { label: t('nearest_enemy', lang), color: 'bg-sky-950/80 text-sky-300 border-sky-700/60' };
      case 'MOUSE_DIRECTION':
        return { label: t('mouse_aim', lang), color: 'bg-purple-950/80 text-purple-300 border-purple-700/60' };
      case 'AREA_OF_EFFECT':
        return { label: t('aoe_label', lang), color: 'bg-emerald-950/80 text-emerald-300 border-emerald-700/60' };
      default:
        return { label: t('weapon_label', lang), color: 'bg-slate-800 text-slate-300 border-slate-600' };
    }
  };

  const lang = getLanguage();
  const levelTitleTemplate = t('level_reached_title', lang) !== 'level_reached_title'
    ? t('level_reached_title', lang)
    : 'LEVEL {level} REACHED!';
  const levelTitle = levelTitleTemplate.replace('{level}', String(level));
  const levelDesc = t('level_reached_desc', lang) !== 'level_reached_desc'
    ? t('level_reached_desc', lang)
      ...
      <div className={`w-full max-h-[94vh] overflow-y-auto bg-gradient-to-b from-slate-900 via-indigo-950/90 to-slate-950 border-2 border-purple-600/70 shadow-2xl shadow-purple-950/60 relative ${
        ...
        <div className={`flex flex-col relative transition-all duration-300 ease-out ${
          ...
          {options.map((opt, index) => {
            ...
            if (opt.kind === 'WEAPON_NEW') {
              title = translateWeaponName(opt.definition.id, opt.definition.name, lang);
              subtitle = t('new_weapon_acquired', lang);
              const tier1 = opt.definition.tiers[0];
              description = tier1 ? translateWeaponTierDescription(opt.definition.id, tier1.tier, tier1.description, lang) : translateWeaponDescription(opt.definition.id, opt.definition.description, lang);
              ...
              tierTag = <span className="bg-emerald-950/90 text-emerald-300 border border-emerald-500/50 text-[10px] sm:text-[11px] font-bold px-1.5 py-0.2 rounded-md flex items-center gap-1"><PlusCircle className="w-3 h-3" /> {t('new_tag', lang)}</span>;
            } else if (opt.kind === 'WEAPON_UPGRADE') {
              title = translateWeaponName(opt.definition.id, opt.definition.name, lang);
              const nextTierObj = opt.definition.tiers.find((t) => t.tier === opt.nextTier);
              subtitle = nextTierObj ? translateWeaponTierName(opt.definition.id, opt.nextTier, nextTierObj.name, lang) : `Tier ${opt.nextTier}`;
              description = nextTierObj ? translateWeaponTierDescription(opt.definition.id, opt.nextTier, nextTierObj.description, lang) : 'Increases weapon prowess.';
              IconComp = WEAPON_ICONS[opt.definition.icon] || Sparkles;
              iconColor = opt.definition.iconColor || opt.definition.bulletColor;
              typeBadge = getShootingTypeLabel(opt.definition.shootingType);
              if (opt.nextTier === 7) {
                tierTag = <span className="bg-amber-950/90 text-amber-300 border border-amber-500/80 text-[10px] sm:text-[11px] font-bold px-1.5 py-0.2 rounded-md flex items-center gap-1 shadow-sm"><Sparkles className="w-3 h-3 text-amber-400" /> {t('mega_evolution', lang)}</span>;
              } else {
              ...
              title = translateStatItemName(opt.definition.id, opt.definition.name, lang);
              subtitle = t('new_stat_artifact', lang);
              const tier1 = opt.definition.tiers[0];
              description = tier1 ? translateStatItemTierDescription(opt.definition.id, tier1.tier, tier1.description, lang) : translateStatItemDescription(opt.definition.id, opt.definition.description, lang);
              IconComp = STAT_ICONS[opt.definition.icon] || Droplet;
              iconColor = opt.definition.color || '#eab308';
              typeBadge = { label: t('passive_artifact', lang), color: 'bg-amber-950/80 text-amber-300 border-amber-700/60' };
              tierTag = <span className="bg-emerald-950/90 text-emerald-300 border border-emerald-500/50 text-[10px] sm:text-[11px] font-bold px-1.5 py-0.2 rounded-md flex items-center gap-1"><PlusCircle className="w-3 h-3" /> {t('new_tag', lang)}</span>;
            } else if (opt.kind === 'STAT_UPGRADE') {
              title = translateStatItemName(opt.definition.id, opt.definition.name, lang);
              const nextTierObj = opt.definition.tiers.find((t) => t.tier === opt.nextTier);
              subtitle = nextTierObj ? translateStatItemTierName(opt.definition.id, opt.nextTier, nextTierObj.name, lang) : `Tier ${opt.nextTier}`;
              description = nextTierObj ? translateStatItemTierDescription(opt.definition.id, opt.nextTier, nextTierObj.description, lang) : 'Amplifies passive artifact power.';
              IconComp = STAT_ICONS[opt.definition.icon] || Droplet;
              iconColor = opt.definition.color || '#eab308';
              typeBadge = { label: t('passive_artifact', lang), color: 'bg-amber-950/80 text-amber-300 border-amber-700/60' };
              tierTag = <span className="bg-amber-950/90 text-amber-300 border border-amber-500/50 text-[10px] sm:text-[11px] font-bold px-1.5 py-0.2 rounded-md flex items-center gap-1"><ArrowUpCircle className="w-3 h-3" /> LVL {opt.currentTier} → {opt.nextTier}</span>;
            ...
            const isLegendary = Boolean(opt.definition.isLegendary || ('nextTier' in opt && opt.nextTier === 7));
        ...
        {!isReady && (
          <div className="absolute inset-x-0 bottom-12 flex flex-col items-center justify-center text-purple-300 pointer-events-none animate-pulse">
            ...
            <span className="text-xs font-semibold tracking-wide">{t('channeling_arcane', getLanguage())}</span>
          </div>
        )}
        ...
        {canReroll && onReroll && isReady && (
          <div className="flex justify-center mt-3">
            ...
            >
              ...
              <span>{t('reroll_options', getLanguage())}</span>
            </button>
          </div>
        )}
        ...
        <div className={`text-center text-slate-400 ${mobileMode ? 'mt-2 text-[10px] sm:text-xs' : 'mt-4 text-xs'}`}>
          {isReady ? (
            ...
            <span className="text-purple-400 font-medium inline-flex items-center gap-1.5 animate-pulse">
              <Sparkles className="w-3 h-3 text-amber-400" /> {t('channeling_boons', getLanguage())}
            </span>
          )}
        </div>
      </div>
